...
const RAW: GlossaryTerm[] = [
  ...
  // ======== GATES & CIRCUITS (expansion) ========
  {
    term: 'Toffoli gate',
    aliases: ['CCNOT', 'controlled-controlled-NOT'],
    category: 'gates',
    audience: 'intermediate',
    short: 'A three-qubit gate that flips the target only when both controls are 1.',
    long: 'Toffoli is classically universal for reversible computation and a key primitive for quantum arithmetic. It is non-Clifford and typically decomposed into 6 CNOTs and 7 T-gates, making it expensive under fault tolerance.',
    whyItMatters: 'Arithmetic-heavy algorithms like Shor\'s depend on efficient Toffoli implementations; T-count optimization of Toffolis is a major compiler concern.',
    related: ['T gate', 'CNOT', "Shor's algorithm", 'Magic state distillation'],
  },
  {
    term: 'SWAP gate',
    category: 'gates',
    audience: 'intermediate',
    short: 'A gate that exchanges the states of two qubits.',
    long: 'SWAP decomposes into three CNOTs and is required for routing logical operations on hardware with limited connectivity. SWAP overhead is a major reason heavy-hex or nearest-neighbor architectures yield lower effective performance than all-to-all platforms.',
    whyItMatters: 'A device\'s connectivity graph determines how many SWAPs a real circuit needs; SWAP-heavy circuits inflate depth and erode fidelity.',
    related: ['CNOT', 'Connectivity', 'Circuit depth'],
  },
  {
    term: 'S gate',
    aliases: ['phase gate', '√Z'],
    category: 'gates',
    audience: 'intermediate',
    short: 'A single-qubit gate that applies a 90° phase rotation around the Z axis.',
    long: 'S = T² and is a member of the Clifford group, meaning it can typically be implemented transversally in QEC codes at low cost. It appears throughout the QFT and stabilizer-based protocols.',
    related: ['T gate', 'Hadamard gate', 'Quantum Fourier transform'],
  },
  {
    term: 'Universal gate set',
    category: 'gates',
    audience: 'intermediate',
    short: 'A finite collection of gates that can approximate any quantum operation to arbitrary precision.',
    long: 'Common choices are {H, T, CNOT} or {Clifford + T}. The Solovay-Kitaev theorem guarantees efficient approximation, with polylogarithmic gate overhead relative to the desired precision.',
    whyItMatters: 'Hardware vendors must demonstrate a universal set to claim general-purpose computation rather than a special-purpose accelerator.',
    related: ['Clifford+T', 'T gate', 'Hadamard gate', 'CNOT'],
  },
  {
    term: 'Clifford+T',
    category: 'gates',
    audience: 'advanced',
    short: 'The standard universal gate set used in fault-tolerant quantum computing.',
    long: 'The Clifford group (H, S, CNOT) is efficiently classically simulable; adding the T-gate makes the set universal but introduces magic state distillation overhead. "T-count" and "T-depth" are the principal resource metrics in FTQC compilation.',
    whyItMatters: 'Nearly every credible fault-tolerant resource estimate (Shor\'s, chemistry, finance) is quoted in T-count terms.',
    related: ['T gate', 'Magic state distillation', 'Fault-tolerant quantum computing'],
  },
  {
    term: 'Circuit depth',
    category: 'gates',
    audience: 'intermediate',
    short: 'The number of sequential gate layers in a circuit; parallelizable gates count as one layer.',
    long: 'Depth is the length of the longest critical path from input to output. A circuit deeper than ~T2/gate-time becomes uncomputable on a given device regardless of qubit count, so reducing depth is a primary compiler objective.',
    whyItMatters: 'Depth and coherence time together determine what algorithms are feasible on a chip; doubling depth on a noisy device can collapse signal entirely.',
    related: ['T2', 'Gate time', 'Quantum volume'],
  },
  {
    term: 'Ancilla qubit',
    category: 'gates',
    audience: 'intermediate',
    short: 'A helper qubit used to assist a computation but not carry the final answer.',
    long: 'Ancillas are auxiliary qubits used for syndrome extraction, phase kickback, and reversible "uncomputation" of intermediate state. Proper uncomputation is required to avoid garbage entanglement that disturbs interference and breaks the algorithm.',
    whyItMatters: 'Ancilla overhead is a non-trivial fraction of total qubit budget in fault-tolerant designs; modern QEC architectures dedicate large blocks of qubits to syndrome ancillas.',
    related: ['Logical qubit', 'Surface code', 'Magic state distillation'],
  },
  ...
  // ======== ALGORITHMS (expansion) ========
  {
    ...
    category: 'algorithms',
    audience: 'intermediate',
    short: 'The first algorithm to show a quantum computer can be exponentially faster than a classical one on a specific problem.',
    long: 'It decides whether an oracle function is constant or balanced with a single quantum query versus 2^(n-1)+1 deterministic classical queries in the worst case. The impact is pedagogical rather than practical, but it established quantum query complexity as a field and is widely used to benchmark early devices.',
    related: ['Bernstein-Vazirani algorithm', 'Hadamard gate'],
  },
  {
    term: 'Bernstein-Vazirani algorithm',
    category: 'algorithms',
    audience: 'intermediate',
    short: 'A quantum algorithm that finds a hidden bit string in one oracle query instead of n.',
    long: 'Given an oracle f(x) = s·x mod 2, BV recovers the secret string s in a single quantum query versus n classical ones. It cleanly illustrates phase kickback and set the stage for Simon\'s and Shor\'s algorithms.',
    related: ['Deutsch-Jozsa algorithm', "Shor's algorithm", 'Hadamard gate'],
  },
  {
    ...
    category: 'algorithms',
    audience: 'advanced',
    short: 'A subroutine that estimates the eigenphase of a unitary operator U.',
    long: 'QPE uses controlled applications of U and an inverse QFT to extract φ from U|ψ⟩ = e^(2πiφ)|ψ⟩ with precision scaling as 1/N for N ancilla qubits. It is the workhorse inside Shor\'s algorithm, HHL, and chemistry-relevant simulation algorithms.',
    whyItMatters: 'QPE T-counts drive most FTQC resource estimates for chemistry, materials, and cryptanalysis.',
    related: ['Quantum Fourier transform', "Shor's algorithm", 'HHL algorithm'],
  },
  {
    term: 'HHL algorithm',
    aliases: ['Harrow-Hassidim-Lloyd'],
    category: 'algorithms',
    audience: 'advanced',
    short: 'A quantum algorithm that produces a state proportional to the solution of Ax = b in time polylogarithmic in the system size.',
    long: 'HHL works under sparsity and condition-number assumptions and outputs a quantum state, not a classical vector. The quantum-to-classical readout step often dominates runtime, narrowing the regime of practical advantage and triggering many "dequantization" results.',
    whyItMatters: 'HHL motivates quantum machine learning claims; investor due diligence should ask about input/output caveats before accepting exponential-speedup pitches.',
    related: ['Quantum machine learning', 'Quantum phase estimation', 'Quantum Fourier transform'],
  },
  {
    term: 'Amplitude amplification',
    category: 'algorithms',
    audience: 'advanced',
    short: 'A generalization of Grover\'s algorithm that quadratically amplifies the probability of marked states.',
    long: 'Given any state-preparation oracle and a marking predicate, amplitude amplification finds a marked outcome with O(√(1/p)) calls versus O(1/p) classically. It is the foundational subroutine behind quantum Monte Carlo speedups, Grover, and several QML primitives.',
    whyItMatters: 'Powers proposed financial-modeling and risk-pricing speedups (Monte Carlo); the speedup is provable but quadratic, not exponential.',
    related: ["Grover's algorithm", 'Quantum machine learning'],
  },
  {
    term: 'Quantum walk',
    category: 'algorithms',
    audience: 'advanced',
    short: 'The quantum analogue of a random walk; provides quadratic or higher speedups for graph problems.',
    long: 'Discrete- or continuous-time quantum walks evolve a particle on a graph via unitary dynamics, achieving speedups for element distinctness, Markov chain mixing, and triangle finding. Quantum walks are increasingly used as building blocks for algorithm design.',
    related: ["Grover's algorithm", 'Amplitude amplification'],
  },
  {
    ...
    category: 'algorithms',
    audience: 'intermediate',
    short: 'Using quantum computers as primitives inside machine-learning pipelines.',
    long: 'QML encompasses quantum kernels, variational classifiers, quantum neural networks, and HHL-based linear-algebra speedups. Many proposed speedups have classical "dequantization" counterexamples, so practical advantage remains an open empirical question.',
    whyItMatters: 'Heavily marketed, least empirically proven near-term category; treat exponential-speedup QML pitches with skepticism unless the input/output model is fully specified.',
    related: ['HHL algorithm', 'Variational Quantum Eigensolver', 'QAOA'],
  },
  ...
  // ======== HARDWARE (expansion) ========
  {
    ...
    category: 'hardware',
    audience: 'advanced',
    short: 'A bosonic qubit encoded in Schrödinger-cat superpositions of a microwave oscillator; bit-flips are exponentially suppressed by design.',
    long: 'Cat qubits exploit the symmetry of coherent-state superpositions to exponentially suppress bit-flip errors in the oscillator energy, leaving phase-flips as the dominant error channel. The resulting biased noise dramatically reduces fault-tolerant overhead, and is the foundation of Alice & Bob\'s and AWS\'s architectures.',
    whyItMatters: 'Biased-noise QEC can cut total qubit overhead by ~10x versus surface code at unbiased noise; a key reason cat-qubit roadmaps target 100 logical qubits with thousands rather than millions of physical qubits.',
    related: ['Bosonic qubit', 'Surface code', 'Fault-tolerant quantum computing'],
  },
  {
    term: 'Bosonic qubit',
    category: 'hardware',
    audience: 'advanced',
    short: 'A qubit encoded in the infinite-dimensional state space of a quantum oscillator.',
    long: 'Bosonic encodings (cat, GKP, binomial, dual-rail) use a single oscillator mode to host a protected logical qubit, providing intrinsic redundancy without many physical qubits. They form a key path to "hardware-efficient" error correction explored by AWS, Alice & Bob, Quantum Circuits Inc., and Xanadu.',
    related: ['Cat qubit', 'Photonic qubit', 'Quantum error correction'],
  },
  {
    term: 'Rydberg atom',
    category: 'hardware',
    audience: 'advanced',
    short: 'An atom excited to a high principal quantum number, producing strong, switchable interactions used for entangling gates.',
    long: 'Rydberg states have large electric dipole moments and strong van der Waals interactions. The "Rydberg blockade" prevents two nearby atoms from being simultaneously excited, providing a natural conditional gate mechanism that QuEra, Pasqal, Atom Computing, and Infleqtion exploit.',
    whyItMatters: 'The blockade mechanism enables neutral-atom two-qubit gates without trapping a separate ion bus, which is why neutral atoms have scaled qubit counts faster than any other modality.',
    related: ['Neutral atom', 'Optical tweezer'],
  },
  {
    term: 'Optical tweezer',
    category: 'hardware',
    audience: 'intermediate',
    short: 'A focused laser beam used to trap and reposition a single atom.',
    long: 'Tightly focused beams create dipole-force potentials that hold individual neutral atoms; arrays are generated by acousto-optic deflectors or spatial light modulators. The trap geometry is fully software-reconfigurable, enabling 2D and 3D atomic arrays with arbitrary topology.',
    whyItMatters: 'Enabling technology behind 1,000+ qubit neutral-atom systems (QuEra Gemini, Atom Computing Phoenix); reconfigurability gives a connectivity advantage over fixed lithographic chips.',
    related: ['Neutral atom', 'Rydberg atom'],
  },
  {
    term: 'Cryostat',
    category: 'hardware',
    audience: 'intermediate',
    short: 'The general category of cryogenic enclosure that cools quantum hardware.',
    long: 'Cryostats include dilution refrigerators (sub-20 mK for transmons), pulse-tube coolers, and liquid-helium dewars. Different qubit platforms require different temperatures, from ~4 K for some photonic detectors down to ~10 mK for superconducting chips.',
    related: ['Dilution refrigerator', 'Superconducting qubit'],
  },
  {
    term: 'Sycamore',
    category: 'hardware',
    audience: 'intermediate',
    short: 'Google\'s 53-qubit superconducting chip that claimed the first quantum supremacy demonstration in 2019.',
    long: 'Sycamore performed a random-circuit-sampling task Google estimated would take a classical supercomputer 10,000 years, later contested as days–weeks with improved classical methods. It remains the field\'s watershed moment for public attention.',
    related: ['Quantum supremacy', 'Willow', 'Superconducting qubit'],
  },
  {
    term: 'Willow',
    category: 'hardware',
    audience: 'intermediate',
    short: 'Google\'s 105-qubit chip that demonstrated below-threshold quantum error correction in 2024.',
    long: 'Willow showed that increasing surface-code distance from 3 to 5 to 7 reduced logical error rates exponentially, the first convincing empirical demonstration that QEC actually suppresses errors at chip scale. It is widely viewed as the most important QEC milestone to date.',
    whyItMatters: 'Empirical evidence the fault-tolerance scaling thesis works; a major de-risking event for the FTQC investment case.',
    related: ['Surface code', 'Threshold theorem', 'Sycamore'],
  },
  {
    term: 'Heron',
    category: 'hardware',
    audience: 'intermediate',
    short: 'IBM\'s 156-qubit superconducting flagship chip featuring tunable couplers and record layer fidelities.',
    long: 'Heron significantly reduces crosstalk versus prior Eagle/Osprey chips and delivers IBM\'s best-published two-qubit and layer fidelities. It is the foundation IBM uses to demonstrate "quantum utility" on circuits beyond brute-force classical simulation.',
    related: ['Layer fidelity', 'Quantum utility', 'Starling'],
  },
  {
    term: 'Helios',
    category: 'hardware',
    audience: 'intermediate',
    short: 'Quantinuum\'s next-generation trapped-ion system targeting ~96 high-fidelity qubits with all-to-all connectivity.',
    long: 'Helios builds on the QCCD architecture and the H1/H2 lineage of record-breaking fidelity benchmarks, slated for delivery in 2025. It is widely viewed as the leading near-term candidate for early logical-qubit color-code demonstrations.',
    related: ['Trapped ion', 'Color code', 'Apollo'],
  },
  {
    term: 'Apollo',
    category: 'hardware',
    audience: 'advanced',
    short: 'Quantinuum\'s roadmap system aimed at fully fault-tolerant trapped-ion quantum computing later this decade.',
    long: 'Apollo is positioned as Quantinuum\'s first FTQC-class machine, scaling beyond Helios with hundreds of logical qubits. It anchors Quantinuum\'s enterprise pitch on a credible commercial-FTQC date.',
    related: ['Helios', 'Fault-tolerant quantum computing', 'Color code'],
  },
  {
    term: 'Starling',
    category: 'hardware',
    audience: 'advanced',
    short: 'IBM\'s roadmap system designed to be its first fault-tolerant quantum computer (~200 logical qubits, 2029).',
    long: 'Starling leverages qLDPC codes and modular interconnects, is the centerpiece of IBM\'s 2024-published 10-year roadmap, and follows Heron in the superconducting product line. Its delivery date is the most-watched milestone for commercial FTQC timelines.',
    related: ['qLDPC codes', 'Heron', 'Logical qubit'],
  },
  {
    term: 'Tempo',
    category: 'hardware',
    audience: 'intermediate',
    short: 'IonQ\'s next-generation trapped-ion system targeting commercial data-center deployment.',
    long: 'Tempo is positioned for enterprise on-prem and cloud deployment, with #AQ targets in the high tens and networked architecture for multi-rack scaling. It reflects IonQ\'s shift from research-platform sales to productized commercial systems.',
    related: ['Forte', 'Algorithmic qubits', 'Trapped ion'],
  },
  {
    term: 'Forte',
    category: 'hardware',
    audience: 'intermediate',
    short: 'IonQ\'s current-generation 32-qubit ytterbium trapped-ion system.',
    long: 'Forte and Forte Enterprise (the rack-mounted variant) are IonQ\'s reference platforms for its #AQ-benchmarked commercial offering. They are deployed at AWS Braket, Azure Quantum, and Google Cloud, and on-premises with Hyundai Motor and Airbus.',
    related: ['Tempo', 'Algorithmic qubits', 'Trapped ion'],
  },
  {
    term: 'Aurora',
    category: 'hardware',
    audience: 'advanced',
    short: 'Xanadu\'s modular photonic quantum computing system networked through fiber.',
    long: 'Aurora connects dozens of squeezed-light photonic chips via fiber to demonstrate scalable modular architecture, targeting fault tolerance through Gottesman-Kitaev-Preskill (GKP) bosonic encoding. It is the most-advanced public demonstration of room-temperature modular photonic QC.',
    related: ['Photonic qubit', 'Bosonic qubit'],
  },
  ...
  // ======== ERROR CORRECTION (expansion) ========
  {
    ...
    category: 'error_correction',
    audience: 'advanced',
    short: 'A topological stabilizer code defined on three-colorable lattices, supporting transversal Clifford gates.',
    long: 'Color codes have lower thresholds than surface codes (~0.1–0.6% depending on variant) but allow the entire Clifford group to be applied transversally, which dramatically reduces magic-state-distillation overhead. They are increasingly central to trapped-ion FTQC roadmaps where higher connectivity is available.',
    whyItMatters: 'Quantinuum\'s Helios/Apollo roadmap leans on color codes; ion-trap connectivity makes them practical where superconducting heavy-hex layouts struggle.',
    related: ['Surface code', 'Transversal gate', 'Helios'],
  },
  {
    term: 'Steane code',
    category: 'error_correction',
    audience: 'advanced',
    short: 'The original 7-qubit CSS code, correcting any single-qubit error and supporting transversal Cliffords.',
    long: 'Derived from the classical [7,4,3] Hamming code, the Steane [[7,1,3]] code is a CSS construction and a pedagogical workhorse for understanding stabilizer formalism. Small-scale demonstrations on neutral-atom and trapped-ion devices have been published.',
    related: ['Surface code', 'Transversal gate', 'Quantum error correction'],
  },
  {
    term: 'Magic state',
    category: 'error_correction',
    audience: 'advanced',
    short: 'A special quantum state injected to perform non-Clifford gates inside fault-tolerant computations.',
    long: 'Magic states like |T⟩ = (|0⟩ + e^(iπ/4)|1⟩)/√2 enable T-gate teleportation into a Clifford-only fault-tolerant computation. High-fidelity magic states are produced by distillation, which dominates FTQC chip area and runtime.',
    whyItMatters: 'Magic-state factories are the single largest line item in most credible FTQC resource estimates.',
    related: ['Magic state distillation', 'T gate', 'Clifford+T'],
  },
  {
    term: 'Transversal gate',
    category: 'error_correction',
    audience: 'advanced',
    short: 'A logical gate implemented by applying the same physical operation to each qubit of a code block.',
    long: 'Transversal gates prevent errors from spreading between qubits within a code block, providing the cleanest form of fault tolerance. The Eastin-Knill theorem proves no code can support a transversal universal gate set, which is why magic-state injection is required.',
    related: ['Color code', 'Magic state', 'Fault-tolerant quantum computing'],
  },
  {
    term: 'Code distance',
    category: 'error_correction',
    audience: 'advanced',
    short: 'The minimum weight of an undetectable error; a code corrects up to ⌊(d-1)/2⌋ errors.',
    long: 'Logical error rates scale roughly as (p/p_th)^(d/2) below threshold, so doubling the distance can reduce logical errors by orders of magnitude. For the surface code, doubling distance costs ~4x physical qubits and ~2x time per QEC cycle.',
    whyItMatters: 'Code-distance progress is what Willow\'s d=3/5/7 demonstration tracked; it is the cleanest empirical test of the threshold theorem.',
    related: ['Threshold theorem', 'Surface code', 'Willow'],
  },
  {
    term: 'Syndrome measurement',
    category: 'error_correction',
    audience: 'advanced',
    short: 'Measuring stabilizer operators via ancilla qubits to diagnose errors without collapsing the logical state.',
    long: 'Syndrome extraction yields classical bits that a decoder maps to the most-likely error, which is then either applied or tracked in software. Syndrome circuits run continuously during fault-tolerant computation and are the dominant workload on a QEC-era chip.',
    related: ['Ancilla qubit', 'Surface code', 'Decoder'],
  },
  ...
  // ======== METRICS (expansion) ========
  {
    ...
    category: 'metrics',
    audience: 'intermediate',
    short: 'IBM\'s 2023 framing for circuits beyond brute-force classical simulation, even if not strictly faster than the best classical methods.',
    long: '"Utility" describes the regime where quantum devices yield trustworthy results on circuits classical methods cannot reproduce exactly, distinguishing it from "supremacy" (any task, useful or not) and "advantage" (faster on a useful task). It is a deliberately more rigorous, less hyped intermediate target.',
    whyItMatters: 'A useful investor framing: utility is achievable now on Heron-class hardware; advantage on commercially meaningful problems is still ahead.',
    related: ['Quantum advantage', 'Quantum supremacy', 'Heron'],
  },
  {
    term: 'Layer fidelity',
    category: 'metrics',
    audience: 'advanced',
    short: 'An IBM-defined benchmark estimating the fidelity of a full simultaneous gate layer, including crosstalk effects.',
    long: 'Layer fidelity captures parallel-execution effects that isolated two-qubit benchmarks miss, providing a more honest reflection of large-scale performance. IBM reports Heron layer fidelities >99.9% on chains of 100+ qubits.',
    whyItMatters: 'Single-pair fidelities can mask crosstalk that only appears at scale; layer fidelity is the more credible apples-to-apples metric.',
    related: ['Gate fidelity', 'Quantum volume', 'Heron'],
  },
  {
    term: 'Readout fidelity',
    category: 'metrics',
    audience: 'intermediate',
    short: 'How accurately a measurement assigns the correct 0 or 1 outcome to a qubit.',
    long: 'Readout fidelity is typically quoted per qubit and per shot, often the slowest and noisiest operation on a chip. Improving it directly accelerates syndrome extraction in QEC and enables mid-circuit measurement.',
    related: ['SPAM fidelity', 'Measurement', 'Syndrome measurement'],
  },
  {
    term: 'Coherence time',
    category: 'metrics',
    audience: 'newbie',
    short: 'The total time a qubit stays "quantum" before noise destroys the information.',
    long: 'Loosely the minimum of T1 and T2, coherence time is most useful expressed as a ratio to gate time, since that ratio bounds usable circuit depth. Trapped ions sustain seconds of coherence; transmons hundreds of microseconds; photons nanoseconds in storage.',
    whyItMatters: 'Coherence-time-to-gate-time ratio is a fairer single-number platform comparison than either alone.',
    related: ['T1', 'T2', 'Gate time'],
  },
  {
    term: 'Gate time',
    category: 'metrics',
    audience: 'intermediate',
    short: 'How long a single quantum gate takes to execute.',
    long: 'Typical gate times: ~10–50 ns for superconducting transmons, ~10–100 μs for trapped ions, ~100 ns–μs for neutral atoms. Faster gates favor variational and short-depth algorithms; slower-but-cleaner gates favor long fault-tolerant circuits.',
    related: ['Coherence time', 'Circuit depth', 'CLOPS'],
  },
  {
    term: 'Connectivity',
    aliases: ['heavy-hex', 'all-to-all', 'nearest-neighbor'],
    category: 'metrics',
    audience: 'intermediate',
    short: 'Which qubits can directly interact via native two-qubit gates.',
    long: 'IBM superconducting chips use heavy-hex; trapped ions within a single zone are all-to-all; most other chips are nearest-neighbor on a 2D grid. Lower connectivity forces SWAP overhead that inflates circuit depth and erodes effective performance.',
    whyItMatters: 'Two devices with identical qubit counts can have vastly different effective compute power; connectivity is a first-order parameter for honest cross-platform comparison.',
    related: ['SWAP gate', 'Circuit depth', 'Quantum volume'],
  },

  // ======== CRYPTOGRAPHY (expansion) ========
  {
    term: 'ML-DSA',
    aliases: ['CRYSTALS-Dilithium', 'FIPS 204'],
    category: 'crypto',
    audience: 'intermediate',
    short: 'The NIST-standardized lattice-based digital signature scheme (FIPS 204, August 2024).',
    long: 'ML-DSA replaces RSA and ECDSA for digital signatures and is the default PQC signature algorithm in NSA CNSA 2.0. It uses module-lattice problems and is the partner to ML-KEM in PQC migration planning.',
    whyItMatters: 'Code-signing, PKI, and authentication migration to ML-DSA is a multi-year coordinated project; it is the second-most-cited PQC primitive after ML-KEM.',
    related: ['ML-KEM', 'SLH-DSA', 'NIST PQC', 'Post-quantum cryptography'],
  },
  {
    term: 'SLH-DSA',
    ...
    category: 'crypto',
    audience: 'advanced',
    short: 'A backup quantum-safe signature scheme based only on hash functions (FIPS 205).',
    long: 'SLH-DSA offers conservative security relying solely on the security of hash functions, at the cost of larger signatures and slower performance. It serves as defense-in-depth if lattice assumptions are ever weakened by future cryptanalysis.',
    related: ['ML-DSA', 'NIST PQC', 'Post-quantum cryptography'],
  },
  {
    term: 'Lattice cryptography',
    category: 'crypto',
    audience: 'advanced',
    short: 'The mathematical foundation behind ML-KEM, ML-DSA, and most modern PQC standards.',
    long: 'Lattice-based cryptography rests on hardness assumptions like Learning With Errors (LWE) and Module-LWE, which are believed hard for both classical and quantum algorithms. It also underlies fully homomorphic encryption and many advanced cryptographic primitives.',
    whyItMatters: 'A single major lattice cryptanalysis result could disrupt the entire PQC migration; understanding the assumption landscape is a real risk-management exercise.',
    related: ['ML-KEM', 'ML-DSA', 'Post-quantum cryptography'],
  },
  {
    term: 'FIPS 140-3',
    category: 'crypto',
    audience: 'advanced',
    short: 'The U.S. federal validation standard for cryptographic modules.',
    long: 'FIPS 140-3 is the framework under which cryptographic hardware and software modules are certified for federal procurement, with PQC algorithm support being added through new implementation guidance. Vendor certification is the practical compliance gate for selling crypto products into government.',
    related: ['NIST PQC', 'NSA CNSA 2.0', 'ML-KEM'],
  },
  {
    term: 'E91',
    category: 'crypto',
    audience: 'advanced',
    short: 'Artur Ekert\'s 1991 QKD protocol based on entangled pairs and Bell inequality violations.',
    long: 'E91 distributes entangled pairs to two parties and certifies security through observation of Bell-inequality violations, making it the foundation for device-independent QKD variants. It is the conceptual basis for satellite QKD demonstrations such as Micius.',
    related: ['BB84', 'Quantum key distribution', 'Entanglement', 'Quantum repeater'],
  },
  ...
  // ======== NETWORKING ========
  {
    ...
    category: 'networking',
    audience: 'advanced',
    short: 'A device that extends quantum communication over long distances using entanglement swapping and memories.',
    long: 'Repeaters overcome the exponential photon loss of fiber by storing and swapping entanglement in stages, enabling continental-scale entanglement distribution. Practical repeaters remain a major experimental challenge — first-generation devices use quantum memories and probabilistic links.',
    whyItMatters: 'Required for any "quantum internet" beyond a few hundred kilometers; the bottleneck for cross-continental QKD and distributed quantum computing.',
    related: ['Quantum internet', 'Entanglement distribution', 'Quantum key distribution'],
  },
  {
    term: 'Entanglement distribution',
    category: 'networking',
    audience: 'advanced',
    short: 'Creating and delivering entangled pairs between two remote network nodes.',
    long: 'Distribution can occur over fiber, free-space optical links, or satellite (Micius). Rates, fidelities, and distance scale jointly limit network applications; current records distribute entangled pairs at kHz rates over hundreds of kilometers.',
    related: ['Quantum repeater', 'Quantum internet', 'Quantum key distribution'],
  },
  {
    term: 'Quantum internet',
    category: 'networking',
    audience: 'advanced',
    short: 'A future network interconnecting quantum processors, sensors, and clocks via shared entanglement.',
    long: 'A full quantum internet would enable distributed quantum computing, blind computation, and improved metrology beyond what any single device can achieve. Pilot networks exist in the Netherlands (QuTech), China, and the U.S. (Chicago, NYC, DOE testbeds).',
    related: ['Quantum repeater', 'EuroQCI', 'Entanglement distribution'],
  },
  {
    term: 'EuroQCI',
    aliases: ['European Quantum Communication Infrastructure'],
    category: 'networking',
    audience: 'intermediate',
    short: 'The EU\'s plan for a continent-wide secure quantum communication network.',
    long: 'EuroQCI deploys terrestrial fiber and satellite QKD links across member states, integrated with national PQC strategies. The space segment is partnered with the IRIS² constellation; deployment is funded through the Digital Europe Programme and Connecting Europe Facility.',
    whyItMatters: 'Defines the European quantum networking market and the procurement structure for QKD vendors operating in the EU.',
    related: ['Quantum key distribution', 'Quantum internet'],
  },
  ...
  // ======== SENSING (expansion) ========
  {
    ...
    category: 'sensing',
    audience: 'intermediate',
    short: 'A clock that references discrete atomic transitions; defines the SI second.',
    long: 'Cesium fountain clocks define the SI second to ~10^-16 fractional accuracy and underpin GPS and global timing infrastructure. Optical successors are expected to redefine the second by ~2030.',
    related: ['Optical clock', 'Quantum-enhanced PNT'],
  },
  {
    term: 'OPM',
    aliases: ['Optically-Pumped Magnetometer'],
    category: 'sensing',
    audience: 'intermediate',
    short: 'A laser-pumped atomic vapor sensor that measures tiny magnetic fields without cryogenics.',
    long: 'OPMs detect magnetic fields by monitoring laser-induced changes in atomic polarization, achieving fT-scale sensitivity at room temperature. They are increasingly deployed for MEG brain imaging (Cerca Magnetics, FieldLine), unexploded ordnance, and geophysical surveys.',
    whyItMatters: 'A commercial sweet spot for quantum sensing; the MEG market alone is a near-term revenue opportunity with healthcare reimbursement pathways.',
    related: ['Quantum sensing', 'NV magnetometer'],
  },
  {
    term: 'NV magnetometer',
    category: 'sensing',
    audience: 'advanced',
    short: 'A magnetic sensor based on nitrogen-vacancy defects in diamond, operating from cryo to elevated temperatures.',
    long: 'NV magnetometers use optically detected magnetic resonance to measure local magnetic fields with single-cell spatial resolution. Diamond hosts the sensor at room temperature, enabling compact portable devices unavailable with SQUIDs or OPMs.',
    related: ['NV-center', 'Quantum sensing', 'OPM'],
  },
  {
    term: 'Quantum gravimeter',
    category: 'sensing',
    audience: 'advanced',
    short: 'A device that measures gravitational acceleration using atom interferometry.',
    long: 'Cold-atom interferometers record matter-wave phase shifts under gravity, achieving μGal-scale sensitivity in field-portable prototypes. Applications include oil & gas exploration, civil engineering (void detection), and GPS-denied inertial navigation.',
    whyItMatters: 'Emerging defense and infrastructure tool; UK Birmingham Royal Engineers field trials and AOSense/Muquans/Exail commercial systems are now operational.',
    related: ['Quantum sensing', 'Quantum-enhanced PNT'],
  },
  {
    term: 'Quantum-enhanced PNT',
    aliases: ['PNT'],
    category: 'sensing',
    audience: 'intermediate',
    short: 'Using quantum sensors and clocks to provide positioning, navigation, and timing without GPS.',
    long: 'Combines atomic clocks, gravimeters, magnetometers, and atom-interferometer inertial sensors to maintain navigation in GPS-denied environments. It is a top-priority defense application area under AUKUS Pillar 2 and DoD funding programs.',
    whyItMatters: 'An early-revenue quantum vertical with concrete DoD procurement; less speculative than QC and faster to deploy than QKD networks.',
    related: ['Atomic clock', 'Quantum gravimeter', 'AUKUS Pillar 2'],
  },

  // ======== PROGRAMS (expansion) ========
  {
    term: 'US2QC',
    aliases: ['Underexplored Systems for Utility-Scale Quantum Computing'],
    category: 'programs',
    audience: 'advanced',
    short: 'A DARPA program funding non-mainstream paths to large-scale quantum computers.',
    long: 'US2QC complements QBI by hedging on alternative modalities — funding Microsoft\'s topological program and PsiQuantum\'s photonic effort, among others. Selection signals which long-tail approaches the U.S. government is willing to backstop.',
    related: ['DARPA QBI', 'Topological qubit', 'Photonic qubit'],
  },
  {
    term: 'NQI',
    aliases: ['National Quantum Initiative'],
    category: 'programs',
    audience: 'intermediate',
    short: 'The U.S. law (2018, reauthorized) coordinating and funding federal quantum research.',
    long: 'NQI established the National Quantum Coordination Office and authorized over $1B of federal quantum spending across NIST, DOE, NSF, and DoD. It is the umbrella structure under which most U.S. quantum funding programs operate.',
    related: ['DARPA QBI', 'NIST PQC', 'DOE QIS Centers'],
  },
  {
    term: 'DOE QIS Centers',
    category: 'programs',
    audience: 'intermediate',
    short: 'Five U.S. national-lab-led quantum information science research centers.',
    long: 'Q-NEXT (Argonne), SQMS (Fermilab), QSC (Oak Ridge), C2QA (Brookhaven), and QSA (LBNL) each focus on different aspects of quantum from materials to networking. They are major academic-industrial collaboration hubs and a primary U.S. government R&D vehicle.',
    related: ['NQI', 'Quantum internet'],
  },
  {
    term: 'QED-C',
    aliases: ['Quantum Economic Development Consortium'],
    category: 'programs',
    audience: 'intermediate',
    short: 'A 250+ member consortium coordinating quantum standards, supply chain, and workforce.',
    long: 'Managed by SRI International under NIST authority, QED-C is a primary forum for pre-competitive industry coordination on standards, supply-chain mapping, and workforce development. Its market reports are commonly cited in investor decks.',
    related: ['NQI', 'NIST PQC'],
  },
];
